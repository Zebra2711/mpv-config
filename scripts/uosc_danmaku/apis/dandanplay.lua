...
function get_danmaku_fallback(query)
    ...
    msg.verbose("Attempt to retrieve danmaku:" .. url)
    ...
    call_cmd_async(arg, function(error)
        async_running = false
        if error then
            show_message("The HTTP request failed, please open the console to view the details", 5)
            msg.error(error)
            return
        end
...
function make_danmaku_request_args(method, url, headers, body)
    ...
    if not file_exists(dandanplay_path) then
        show_message("Missing executable file")
        msg.warn(dandanplay_path .. "does not exist")
        return nil
    end
...
local function match_anime()
    ...
    if not episode_num then
        msg.info("Unable to parse episode information.")
        return
    end
