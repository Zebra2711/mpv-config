...
local function query_tmdb(title, class, menu)
    ...
    if res.status ~= 0 or not data.results or #data.results == 0 then
        local message = "Failed to retrieve TMDB Chinese data"
        if uosc_available then
            update_menu_uosc(menu.type, menu.title, message, menu.footnote, menu.cmd, title)
        else
            show_message(message, 3)
        end
        msg.error("Failed to retrieve TMDB Chinese data：" .. res.stdout)
    else
        if class == "tv" then
...
function get_details(class, id, site, title, year, number, episodenum)
    local message = episodenum and "Querying danmu" or "Loading data..."
    local menu_type = "menu_details"
    local menu_title = "Episode info"
    local footnote = "Use / Open Filter"
    ...
    if class == "Movie" then
        cat = 1
    elseif class == "TV/Drama" then
        cat = 2
--  elseif class == "综艺" then
--      cat = 3
    elseif class == "Cartoon" then
        cat = 4
    end
    ...
    if not number or cat == 0 then
        local message = "No results"
        if uosc_available and not episodenum then
            update_menu_uosc(menu_type, menu_title, message, footnote)
        else
            show_message(message, 3)
        end
        msg.verbose("No results")
        return
    end
...
function query_extra(name, class)
    ...
    if options.tmdb_api_key == "" or #base64.decode(options.tmdb_api_key) < 32 then
        local message = "Please correctly set the tmdb_api_key or try searching in Chinese."
        if uosc_available then
            update_menu_uosc(menu.type, menu.title, message, menu.footnote, menu.cmd, name)
        else
            show_message(message, 3)
